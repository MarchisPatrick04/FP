...
Problem statement
    ...
    3. Build a list of primes , a list of negative numbers, a list of the rest of the numbers, a list of capitalized
    names

    4. Print out all prime numbers etc. in ascending order, without
"""

def is_capitalized(word : str) -> bool:
    return 'A' <= word[0] <= 'Z'

print(is_capitalized("anna"))
...
def is_prime(n: int) -> bool:
    ...
    if n % 2 == 0:
        return False
    # range -> start with 3 then +2 at every step untill sqrt(n
...
for token in tokens:
    try:
        ...
        # if there's an error on the previous line, the execution skips directly to the except
        #clause below
        if int_token < 0:
